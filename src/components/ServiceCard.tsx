...
export default function ServiceCard({ service, index }: ServiceCardProps) {
  return (
    <div className={`animate-on-scroll animate-delay-${(index + 1) * 100} bg-white rounded-xl border border-gray-100 overflow-hidden card-hover`}>
      <div className="relative h-48 overflow-hidden">
        <img src={serviceImages[service.icon]} alt={`${service.title} service`} className="w-full h-full object-cover" loading="lazy" />
        <div className="absolute inset-0 bg-gradient-to-t from-navy/80 to-transparent" />
        <div className="absolute bottom-4 left-4 right-4">
          <div className="flex items-center gap-3">
            <div className="bg-cyan p-2 rounded-lg text-white">{serviceIcons[service.icon]}</div>
      ...
      <div className="p-6">
        <ul className="space-y-2 mb-6">
          {service.items.map((item) => (
            <li key={item} className="flex items-center gap-2 text-gray-600 text-sm">
              <svg className="w-4 h-4 text-cyan flex-shrink-0" fill="currentColor" viewBox="0 0 20 20">
        ...
        <div className="flex gap-3">
          <a href={`tel:${siteConfig.contact.phoneRaw}`} className="btn btn-call flex-1 text-sm py-2.5">
            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z" />
            </svg>
            Call Now
          </a>
          <a href={siteConfig.contact.whatsappLink} target="_blank" rel="noopener noreferrer" className="btn btn-whatsapp flex-1 text-sm py-2.5">
